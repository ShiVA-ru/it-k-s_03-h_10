...
import { closeDB, devicesCollection, runDB } from "../src/db/mongo.js";
...
describe("devices.repository", () => {
	...
	it("creates multiple sessions and deleting one by deviceId reduces the count", async () => {
		...
		const countAfterCreate = await devicesCollection.countDocuments({ userId });
		...
		const countAfterDelete = await devicesCollection.countDocuments({ userId });
